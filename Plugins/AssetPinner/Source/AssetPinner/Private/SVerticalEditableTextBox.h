 ...
 */
class SVerticalEditableTextBox : public SBorder
{

public:

	SLATE_BEGIN_ARGS(SVerticalEditableTextBox)
	...
	SVerticalEditableTextBox();
	virtual ~SVerticalEditableTextBox();
	...
	FText GetText() const
	{
		return VerticalEditableText->GetText();
	}
	...
	bool IsReadOnly() const { return VerticalEditableText->IsTextReadOnly(); }

	/** See the IsPassword attribute */
	void SetIsPassword(TAttribute< bool > InIsPassword);

	bool IsPassword() const { return VerticalEditableText->IsTextPassword(); }
...
protected:
	...
	TSharedPtr< SVerticalEditableText > VerticalEditableText;
